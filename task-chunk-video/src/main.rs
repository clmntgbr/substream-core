                        ...
                        match serde_json::from_slice::<TaskMessage<ChunkVideoPayload>>(&delivery.data) {
                            Ok(message) => {
                                ...
                                tokio::spawn(async move {
                                    ...
                                    match process_chunk_video(&message.payload, &message.payload.task_id, &s3_client).await {
                                        Ok(result) => {
...
async fn process_chunk_video(
    ...
    let result = process_chunk_video_inner(payload, s3_client, &temp_dir, &start_time).await;
